...
def generate_threat_profile(threat_data,analysis):
    ...
    # the properties defined in OCTAVE-S (Asset,Access, Actors, Motives, Outcomes); Access and Motives are optional.
    for item in threat_data:
        # Add Access (Optional)
        if item == 'Access' and threat_data[item]:
            graph.node(
                threat_data['Access'], shape="oval", color="lightblue", style="filled")
        # Add Actors
        if item == 'Actors' and  threat_data[item]:
        ...
        # Add Motives (Optional)
        if item == 'Motives' and threat_data[item]:
        ...
        if item == 'Outcomes' and threat_data['Motives'] == {}:
